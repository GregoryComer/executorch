...
/*
 * Partitioning groups the nodes XNNPACK can execute into maximal subgraphs,
 * which are later fused into single delegated nodes. The partitions must form a
 * valid topological cut: two subgraphs may not depend on each other (even
 * transitively through non-delegated nodes), or fusing them would create a
 * cycle.
 *
 * assign_partitions builds the partitions with a topological sweep, pulling
 * ready delegated nodes (by input-readiness) into the current partition. When a
 * delegated node feeds a non-delegated node (it "escapes" the subgraph), the
 * delegated nodes reachable past that boundary are blocked from the current
 * partition and deferred to a later one -- this is what keeps the subgraphs
 * acyclic. A new partition is started once no more nodes can join. Each node's
 * tag holds its partition id (0 = none); fuse_partitions then collapses each
 * partition into one subgraph node.
 */
...
std::optional<NodeHandle> take_from_queue(
    ...
    std::vector<uint32_t>& block_in_partition,
...
void update_block_frontier(
    ...
    Node& node,
    std::vector<uint32_t>& block_in_partition,
    std::vector<uint32_t>& seen_in_partition,
    uint32_t current_partition) {
  std::deque<NodeHandle> queue;
  ...
  while (!queue.empty()) {
    ...
    if (seen_in_partition[handle] == current_partition) {
      continue;
    }
...
runtime::Result<uint32_t> assign_partitions(Graph& graph) {
  uint32_t current_partition_id = 0;
  ...
  std::vector<uint32_t> block_in_partition(graph.nodes.size(), 0);
  std::vector<uint32_t> seen_in_partition(graph.nodes.size(), 0);
  ...
  for (NodeHandle n = 0u; n < graph.nodes.size(); n++) {
    ...
    auto has_nondelegated_user =
        std::any_of(node.users.begin(), node.users.end(), [&](NodeHandle u) {
          return (graph.nodes[u].flags & NodeFlags::UseXnnpack) ==
              NodeFlags::None;
        });
    ...
    in_edges[n] =
        std::count_if(args.begin(), args.end(), [&](const ValueHandle& a) {
          if (a.is_null())
            return false;
          return !std::holds_alternative<InputNode>(
                     graph.nodes[a.node].value) &&
  ...
  while (remaining_delegated_node_count > 0) {
    ...
    ET_CHECK_OR_RETURN_ERROR(
        current_partition_id != std::numeric_limits<uint32_t>::max(),
        Internal,
        "assign_partitions exceeded the maximum partition count");
    ...
    while (true) {
      while (!non_delegated_queue.empty()) {
        ...
        bool is_input =
            std::holds_alternative<InputNode>(graph.nodes[ndh].value) ||
        ...
        for (auto user : graph.nodes[ndh].users) {
          ...
          in_edges[user]--;
          if (in_edges[user] == 0) {
            if ((graph.nodes[user].flags & NodeFlags::UseXnnpack) !=
                NodeFlags::None) {
              if ((graph.nodes[user].flags & NodeFlags::PassInternal1) !=
                  NodeFlags::None) {
    ...
    for (auto handle : deferred) {
      if ((graph.nodes[handle].flags & NodeFlags::PassInternal1) !=
          NodeFlags::None) {
...
runtime::Error fuse_partitions(Graph& graph, uint32_t partition_count) {
  const auto sentinel = std::numeric_limits<NodeHandle>::max();

  for (uint32_t p = 1; p <= partition_count; p++) {
    ...
    std::vector<char> member_mask(graph.nodes.size(), 0);
    for (auto m : members) {
      member_mask[m] = 1;
    }
    auto is_member = [&](NodeHandle h) { return member_mask[h] != 0; };

    // The fused-node remapping below is keyed per node and assumes each member
    // produces a single output, so multi-output nodes are not yet supported.
    for (auto m : members) {
      auto* op = std::get_if<CallOperatorNode>(&graph.nodes[m].value);
      if (op &&
          std::holds_alternative<std::vector<TensorSpec>>(op->output_specs)) {
        ET_LOG(
            Error,
            "Multi-output nodes are not supported in XNNPACK partitions");
    ...
    for (auto m : members) {
      for (auto arg : graph.nodes[m].get_args()) {
        if (arg.is_null())
          continue;
        if (std::holds_alternative<ConstantNode>(graph.nodes[arg.node].value))
          continue;
        if (!is_member(arg.node) &&
            std::find(ext_inputs.begin(), ext_inputs.end(), arg) ==
                ext_inputs.end()) {
    ...
    for (auto m : members) {
      bool external = std::any_of(
          graph.nodes[m].users.begin(),
          graph.nodes[m].users.end(),
    ...
    for (auto m : members) {
      for (auto arg : graph.nodes[m].get_args()) {
        if (arg.is_null())
          continue;
        if (handle_map[arg.node] != sentinel)
          continue;
    ...
    for (auto m : members) {
      ...
      remapped.output_min = op->output_min;
      remapped.output_max = op->output_max;
    ...
    for (NodeHandle n = 0; n < graph.nodes.size(); n++) {
      ...
      for (auto& arg : op->args) {
        ...
        if (output_index[arg.node] != sentinel) {
          arg = ValueHandle{anchor, output_index[arg.node]};
        }
      }
    }
    for (auto& out : graph.outputs) {
      if (output_index[out.node] != sentinel) {
        out = ValueHandle{anchor, output_index[out.node]};
      }
    }
...
runtime::Error partition_xnn_subgraphs(Graph& graph) {
  graph.update_users();
  tag_xnn_nodes(graph);
  ET_UNWRAP(partition_count, assign_partitions(graph));
  ET_CHECK_OK_OR_RETURN_ERROR(fuse_partitions(graph, partition_count));
  ET_CHECK_OK_OR_RETURN_ERROR(graph.compact_nodes());
  return runtime::Error::Ok;
}
