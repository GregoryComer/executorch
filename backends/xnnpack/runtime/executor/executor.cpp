...
runtime::Result<Executor> Executor::build(graph::Graph& graph) {
  ...
  for (size_t n = 0; n < graph.nodes.size(); n++) {
    ...
    for (const auto& aux : cn->tensor->aux_storage) {
      core::Storage borrowed;
      borrowed.data = aux.data;
      borrowed.size_in_bytes = aux.size_in_bytes;
      borrowed.owner = core::StorageOwner::External;
      values[slot].aux_storage.push_back(std::move(borrowed));
    }
  }
