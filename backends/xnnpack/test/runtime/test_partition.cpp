...
TEST(TestPartition, single_node) {
  ...
  graph.update_users();
  auto partition_count_result = assign_partitions(graph);
  ASSERT_TRUE(partition_count_result.ok());
  auto partition_count = *partition_count_result;
...
TEST(TestPartition, diamond_skip_connection) {
  ...
  // Partition 2: add4, add6 (add4 blocked from partition 1; add6 depends on
  // add4).
...
TEST(TestFusePartitions, single_node) {
  ...
  ASSERT_EQ(fuse_partitions(graph, partition_count), Error::Ok);

  // The add node should have been replaced with a CallSubgraphNode.
  EXPECT_TRUE(
      std::holds_alternative<CallSubgraphNode>(graph.nodes[add.node].value));
...
TEST(TestFusePartitions, parallel_multi_output) {
  ...
  EXPECT_TRUE(
      std::holds_alternative<CallSubgraphNode>(graph.nodes[add2.node].value));
  ...
  EXPECT_TRUE(
      std::holds_alternative<std::vector<TensorSpec>>(fused.output_specs));
  ...
  ASSERT_EQ(graph.compact_nodes(), executorch::runtime::Error::Ok);
  ...
  auto& fused_after =
      std::get<CallSubgraphNode>(graph.nodes[graph.outputs[0].node].value);
