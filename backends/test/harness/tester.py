...
class Tester:
    ...
    def _pre(self, stage):
        ...
        if self.cur:
            ...
            assert stage_type in allowed_next_stages, f"Invalid next stage: {stage_type}"
    ...
    def quantize(self, quantize_stage: Optional[Quantize] = None):
        return self._run_stage(quantize_stage or self._get_default_stage(StageType.QUANTIZE), self.example_inputs)

    def export(self, export_stage: Optional[Export] = None):
        return self._run_stage(
            export_stage or self._get_default_stage(StageType.EXPORT, dynamic_shapes=self.dynamic_shapes),
            self.example_inputs,
        )
    ...
    def to_edge_transform_and_lower(
        ...
        return self._run_stage(to_edge_and_transform_stage or self._get_default_stage(StageType.TO_EDGE_TRANSFORM_AND_LOWER))

    def run_passes(self, run_passes_stage: Optional[RunPasses] = None):
        return self._run_stage(run_passes_stage or self._get_default_stage(StageType.RUN_PASSES))

    def partition(self, partition_stage: Optional[Partition] = None):
        return self._run_stage(partition_stage or self._get_default_stage(StageType.PARTITION))

    def to_executorch(self, to_executorch_stage: Optional[ToExecutorch] = None):
        return self._run_stage(to_executorch_stage or self._get_default_stage(StageType.TO_EXECUTORCH))

    def serialize(self, serialize_stage: Optional[Serialize] = None):
        return self._run_stage(serialize_stage or self._get_default_stage(StageType.SERIALIZE))
    ...
    def run_method_and_compare_outputs(
        ...
        print(f"Comparing Stage {stage} with Stage {reference_stage.stage_type()}")
        ...
        for i in range(len(model_output)):
            ...
            if model.dtype == torch.bool:
                ...
                assert torch.allclose(
                    ...
                    f"\tDifference: max: {torch.max(model-ref)}, abs: {torch.max(torch.abs(model-ref))}, mean abs error: {torch.mean(torch.abs(model-ref).to(torch.float))}.\n"
                    f"\t-- Model vs. Reference --\n"
                    f"\t Numel: {model.numel()}, {ref.numel()}\n"
                    f"\tMedian: {model.median()}, {ref.median()}\n"
                    f"\t  Mean: {model.to(torch.float).mean()}, {ref.to(torch.float).mean()}\n"
    ...
    def _calculate_reference_output(
        ...
        #print(f"Computing reference outputs.")
        #print(f"  Inputs: {inputs}")
        #print(f"  Graph: {program}")
        return output, scale
