...
from executorch.backends.xnnpack.test.tester.tester import ToEdgeTransformAndLower, Tester
        ...
        for posargs, inkwargs, _ in ArgumentTupleGenerator(spec).gen():
            ...
            try:
                if isinstance(posargs[0], torch.Tensor):
                    ...
                    if posargs[0].dtype == torch.int8:
                        print("SKIPPING INT8 CASE")
                        continue
...
class FactoTestsXNNPACK(FactoTestsBase):
    def __init__(self, *args, **kwargs):
        super().__init__(Tester, *args, **kwargs)