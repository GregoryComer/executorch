...
class TestPasses(unittest.TestCase):
    ...
    def test_input_spec_prop_cond(self) -> None:  # noqa: C901
        ...
        # We should see the top-level linear weight placeholder and
        # the corresponding submodule inputs tagged with the same
        # placeholder_provenance - pointing to the original top-level
        # placeholder node.

        # Pull the the placeholder node targets from the graph input
        # spec.
        x_input_spec = next(
            spec
        ...
        # Check the submodule placeholders have input specs propagated.
    ...
    def test_input_spec_prop_cond_e2e(self) -> None:
        ...
        # Find the cond node and extract submodules
        true_submodule = None
        false_submodule = None
        ...
        # Verify input specs are present on submodule placeholder nodes
        for node in true_submodule.graph.nodes:
            if node.op == "placeholder":
                input_spec = node.meta.get("input_spec", None)
    ...
    def test_input_spec_prop_map(self) -> None:
        ...
        # Pull the placeholder node targets from the graph input spec.
        x_input_spec = next(
            spec
            for spec in ep.graph_signature.input_specs
            if spec.kind == InputKind.USER_INPUT and spec.arg.name == "x"
